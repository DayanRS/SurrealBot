...
module.exports = {
	...
	async execute(interaction) {
		...
		if(!cringeRole) {
			await interaction.reply(`"Cringe" role does not exist in this server.`);
			return;
		}
		...
		try {
			...
			await interaction.reply(`<@${cringeTarget.id}> (${cringeTarget.id}) is not a member of this server.`);
			return;
		}
		...
		if(guildMemberToCringe.roles.cache.some(role => role.id === cringeRole.id)) {
			...
			await interaction.reply(`<@${guildMemberToCringe.id}> (${guildMemberToCringe.id}) has gotten their Cringe role removed.`);
			
		} else {
			await interaction.guild.members.resolve(cringeTarget).roles.add(cringeRole);
			await interaction.reply(`<@${guildMemberToCringe.id}> (${guildMemberToCringe.id}) has received the Cringe role.`);
		}
	}
};